-- 20260924 — Dos candados sobre la definición de un flujo.
--
-- 1. transicionar_flujo() no publica un flujo con nodos que el motor no sabe
--    ejecutar. execute-workflow los salta con «implementación pendiente» y el
--    run acaba en success: medido el 24/09/2026, "Score AML Automático" tenía un
--    "Congelar Operación" (output:operacion) que no congela nada, y la paleta
--    ofrece una decena de nodos así. Misma familia que la decisión sin
--    configurar (§9.4): algo que parece instalado y no hace nada.
--    Se parte de la definición VIVA (pg_get_functiondef), no del fichero de la
--    migración del 14/08, y se le añade solo el bloque nuevo.
--
-- 2. Un flujo que ya se ejecutó o pasó por revisión NO se borra. Borrar un
--    workflow arrastra en cascada execution_runs, execution_logs,
--    tareas_aprobacion y workflow_autorizaciones: toda la evidencia de sus
--    aprobaciones y de quién lo autorizó. Un control cuya evidencia se puede
--    borrar no es un control (§6.6, §6.7). El que no tiene historia —un
--    borrador recién dibujado— se sigue pudiendo borrar. Para retirar uno con
--    historia: dejarlo en borrador e inactivo.
...
CREATE OR REPLACE FUNCTION public.workflow_borrado_guard()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
REVOKE ALL ON FUNCTION public.workflow_borrado_guard() FROM anon;

DROP TRIGGER IF EXISTS trg_workflow_borrado_guard ON public.workflows;
CREATE TRIGGER trg_workflow_borrado_guard
    BEFORE DELETE ON public.workflows
    FOR EACH ROW EXECUTE FUNCTION public.workflow_borrado_guard();

-- CREATE OR REPLACE conserva los permisos; se reafirman por si acaso (§6.4).
