...
//
// ⚠️ GEMELO COPIADO de src/utils/modelosFinancieros.ts. Si cambias uno, cambia
// el otro. Duplicado por lo mismo que `fecha.ts`, `matriz.ts` y
// `delegaciones.ts` (CLAUDE.md §6.5, §6.6, §9.3): Deno no alcanza `src/`.
//
// ESTE es el que MANDA — es el que clasifica los saldos que salen del nodo. El
// de `src/` solo alimenta el formulario y la vista previa.
//
// Portado de `industryModels.ts` del Sistema de Estados Financieros, que ya
// tenía los dos modelos escritos. Aquí llega partido en dos funciones puras
// (balance y resultado) porque el motor de HermesAI Flow tiene su propio
// pipeline de consolidación y filtrado de hojas, y ese pipeline NO cambia.
// ═══════════════════════════════════════════════════════════════════════════
